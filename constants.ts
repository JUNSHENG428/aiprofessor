...
`,
  ...
  // ===== 框选区域 OCR（结构化提取）=====
  OCR_REGION_STRUCTURED: (intent: 'auto' | 'formula' | 'table' | 'chart' | 'text' = 'auto') => `
你是一位“高精 OCR + 结构化信息抽取”专家。你将收到一张 **框选后的局部图片**（只包含用户关心的区域），请只基于该区域进行识别与抽取。

## 目标
把图片内容抽取为结构化 JSON，用于后续讲解/入库/生成题目。要求：**尽可能准确**，不要脑补。

## 识别重点（按优先级）
1) 数学公式（尽量转为可渲染 LaTeX）
2) 表格（转为 Markdown 表格，保持行列结构）
3) 图表（描述坐标轴/图例/趋势/关键结论）
4) 正文/标题/注释/脚注（按阅读顺序合并）

## 当前意图
${intent === 'auto' ? 'auto（自动识别：公式/表格/图表/文字）' : intent}

## 输出格式（严格 JSON，仅输出 JSON，不要任何其他文字）
\`\`\`json
{
  "text": "按阅读顺序的文字（尽量保留换行）",
  "formulas": [
    { "latex": "E = mc^2", "name": "可选：名称", "confidence": "high" }
  ],
  "tables": [
    { "title": "可选：表格标题", "markdown": "|A|B|\\n|---|---|\\n|...|...|", "confidence": "medium" }
  ],
  "charts": [
    { "description": "图表描述：坐标轴/图例/趋势/数值范围", "keyTakeaways": ["要点1", "要点2"], "confidence": "medium" }
  ],
  "warnings": ["可选：不确定/模糊之处说明（不要瞎猜）"]
}
\`\`\`

## 规则
- 只抽取图片里能看见的内容；不确定就写在 warnings
- 公式 LaTeX 要可渲染：分数用 \\frac，求和用 \\sum 等
- 表格必须是合法 Markdown 表格
`,

  // ===== 跨页公式补全（多图推理）=====
  EXPLAIN_FORMULA_CROSSPAGES: (query: string, pageSnippets: string) => `
你是一位严谨的数学教授。你将收到多页图片（同一份讲义的不同页），以及每页的文字片段。请把关于同一个公式/概念 **跨页的信息**（定义、变量表、推导、例子、注意事项）串起来，给出一份“统一版本”的讲解。
...
## 输出要求（Markdown）
1) 先给出最终“统一版”公式（可渲染 LaTeX）
2) 变量表（符号/含义/单位/来源页码）
3) 推导思路（按页码串起来，说明每一步来自哪页）
4) 一个小例子
5) 易错点/边界条件
6) 若不同页表述有差异，说明差异并给出你判断依据（引用页码）
`,
